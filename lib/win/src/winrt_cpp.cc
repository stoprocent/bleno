...
// Convert std::string to IBuffer
IBuffer stringToIBuffer(const std::string& str) {
    // Get string size and data pointer
    size_t dataSize = str.size();
    const char* data = str.c_str();
    
    // Create a WinRT buffer of appropriate size
    auto buffer = Buffer(static_cast<uint32_t>(dataSize));
...
std::vector<std::string> toPropertyArray(GattCharacteristicProperties& properties)
{
    ...
    SET_VAL(properties, GattCharacteristicProperties::AuthenticatedSignedWrites, "authenticatedSignedWrites")
